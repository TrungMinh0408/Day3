...
<div class="container bg-white shadow rounded p-4">
  <div class="d-flex justify-content-between align-items-center mb-3">
    ...
    <a href="createproduct.php" class="btn btn-success" onclick="return confirm('Bạn chắc chắn muốn thêm sản phẩm mới?')">➕ Add Product</a>
  </div>
  ...
  <form method="GET" class="row g-3 mb-4">
    <div class="col-md-10">
        <div class="input-group">
            <input type="text" class="form-control" name="name" placeholder="🔎 Tìm theo tên sản phẩm..." value="<?php echo htmlspecialchars($searchName); ?>">
            <input type="text" class="form-control" name="category" placeholder="🔎 Tìm theo danh mục..." value="<?php echo htmlspecialchars($searchCategory ?? ''); ?>">
        </div>
    </div>
  ...
  <table class="table table-bordered table-striped table-hover text-center">
    ...
    if ($result && $result->num_rows > 0) {
        while ($row = $result->fetch_assoc()) {
            ...
            echo "<td><img src='" . htmlspecialchars($row['image_url']) . "' class='product-img' alt='image'></td>";
            echo "<td>" . htmlspecialchars($row['category']) . "</td>";
            echo "<td>" . htmlspecialchars($row['created_at']) . "</td>";
            echo "<td><a href='updateproduct.php?id={$row['id']}' class='btn btn-warning btn-sm'>✏️ Sửa</a></td>";
            echo "<td><a href='proccessdeleteProduct.php?id={$row['id']}' class='btn btn-danger btn-sm' onclick=\"return confirm('Bạn có chắc chắn muốn xoá sản phẩm này?')\">🗑️ Xoá</a></td>";
            echo "</tr>";
        }
    } else {
